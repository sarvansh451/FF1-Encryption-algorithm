...
void FF1_encrypt(const unsigned int* in, unsigned int* out, EVP_CIPHER_CTX* evp_enc_ctx, const unsigned char* tweak, const unsigned int radix, size_t inlen, size_t tweaklen)
{
    ...
    for (int i = 0; i < FF1_ROUNDS; ++i) {
        ...
        int tlen;
        EVP_EncryptUpdate(evp_enc_ctx, R, &tlen, P, 16);
        int count = Qlen / 16;
        unsigned char Ri[16];
        unsigned char* Qi = Q;
        for (int cc = 0; cc < count; ++cc) {
            ...
            int tlen;
            EVP_EncryptUpdate(evp_enc_ctx, R, &tlen, Ri, 16);
            Qi += 16;
        }
        ...
        for (int j = 1; j <= cnt; ++j) {
            ...
            int tlen;
            EVP_EncryptUpdate(evp_enc_ctx, SS, &tlen, tmp, 16);
...
int main()
{
    ...
    const int radix = 10;
    const char* plaintext = "0123456789";

    int klen = strlen(key_hex) / 2;
    int tlen = strlen(tweak_hex) / 2;
    int xlen = strlen(plaintext);

    // Allocate memory dynamically
    unsigned char* k = (unsigned char*)malloc(klen);
    unsigned char* t = (unsigned char*)malloc(tlen);
    ...
    hex2chars(key_hex, k);
    hex2chars(tweak_hex, t);
    map_chars(plaintext, x);

    for (int i = 0; i < xlen; ++i)
        assert(x[i] < radix);

    // Create and initialize EVP_CIPHER_CTX
    EVP_CIPHER_CTX* enc_ctx = EVP_CIPHER_CTX_new();

    const EVP_CIPHER* cipher = EVP_aes_128_ecb(); // Using AES-256 for this example

    EVP_EncryptInit_ex(enc_ctx, cipher, NULL, k, NULL);
    ...
    printf("========== FF1 ==========\n");
    FF1_encrypt(x, y, enc_ctx, t, radix, xlen, tlen);
    ...
    FF1_decrypt(y, x, enc_ctx, t, radix, xlen, tlen);
